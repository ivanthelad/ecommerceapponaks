...
helm install stable/nginx-ingress -n myingress
...
helm install --values app.yaml --values inf.yaml --values ingress_values.yaml  --set app.name=$appname  --name="$appname-$infra" ./$infra/
...
do
   ...
   helm install --values app.yaml --values inf.yaml --values ingress_values.yaml  --set app.name=$appname  --name="$appname-$chart" ./$chart/
   # or do whatever with individual element of the array
done
...
kubectl  annotate ingress --all kubernetes.io/ingress.class=nginx --overwrite
kubectl  annotate ingress --all nginx.ingress.kubernetes.io/proxy-buffer-size=16k --overwrite
